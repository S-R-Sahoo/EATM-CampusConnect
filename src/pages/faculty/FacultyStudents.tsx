    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-200/80 shadow-card divide-y divide-gray-100 overflow-hidden">
        {filtered.length === 0 ? (
          ...
          filtered.map(st => (
            <div key={st.id} className="p-4 sm:p-5 flex items-center justify-between gap-4 hover:bg-gray-50 transition">
              <div className="flex items-center gap-3.5">
                <Avatar src={st.photoURL} name={st.displayName} size="md" online={true} />
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="font-bold text-sm text-gray-900">{st.displayName}</h3>
                    <span className="text-[10px] font-bold px-2 py-0.2 rounded-full bg-emerald-100 text-emerald-800">
                      {st.status}
                    </span>
                  </div>
                  <p className="text-xs text-gray-500 mt-0.5">
                    Roll: <strong className="text-gray-700">{st.rollNumber || 'EATM23000'}</strong> • {st.department} • {st.year || '3rd Year'}
                  </p>
